    ...
    private void Update()
    {
        ...
        if (isPlayerNear)
        {
          ...
          //  blockRenderer.material = glowMaterial;
            ...
            if ((Input.GetKeyDown(attachKey) || (buttonpressed == true) )&& player.transform.childCount == 2 /**&& Time.time - lastTransferTime >= transferCooldown**/)
            {
            ...
            else if((Input.GetKeyDown(attachKey) || (buttonpressed == true) )&& player.transform.childCount == 1)
            {
                tempServerRpc();
                attachertemp.GetComponent<Attacher>().buttonpressed = false;
